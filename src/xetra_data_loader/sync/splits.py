...
from xetra_data_loader.gold.splits import SplitGoldResult
...
def sync_splits(
    ...
    *,
    ...
    def mutate(cursor: Cursor[Any]) -> SyncCounters:
        ...
        for key in gold.retracted_keys:
            cursor.execute(
                "DELETE FROM portfell_market.splits "
        ...
        for event in gold.rows:
            cursor.execute(
                "SELECT event_date, split_ratio, split_factor FROM portfell_market.splits "
                "WHERE isin = %s AND exchange = %s AND code = %s AND event_key = %s",
                event.key,
            )
            existing = cast(
            ...
            if existing is None:
                cursor.execute(
                    "INSERT INTO portfell_market.splits "
                ...
                cursor.execute(
                    "UPDATE portfell_market.splits SET event_date = %s, split_ratio = %s, "
