...
def test_cron_contract_is_exact_sunday_1200_vienna() -> None:
...
def test_vienna_1200_remains_local_1200_across_dst() -> None:
    ...
    winter = datetime(2026, 1, 11, 12, 0, tzinfo=vienna)
    summer = datetime(2026, 7, 12, 12, 0, tzinfo=vienna)

    assert winter.weekday() == 6
    assert summer.weekday() == 6
    assert winter.hour == summer.hour == 12
